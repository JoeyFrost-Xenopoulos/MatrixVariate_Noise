#!/usr/bin/env Rscript
# Evaluate RIMLE estimate_k with K-MEANS initialization
  ...
  for (rep in seq_len(n_reps)) {
    ...
    if (is.null(fit_result)) {
      results[[row_idx]] <- list(
        ...
        converged = NA,
        iterations = NA,
        q_final = ifelse(init_method == "hennig-coretto", parse_q_from_verbose(captured), NA)
      )
      row_idx <- row_idx + 1
      next
    }
