...
## ---- matrix_mixture_kmeanspp_init ----

test_that("kmeanspp init returns correct structure", {
  set.seed(42)
  x_list <- lapply(1:15, function(i) matrix(rnorm(6), 2, 3))
  params <- matrix_mixture_kmeanspp_init(x_list, g = 2)
...
test_that("kmeanspp init mixing proportions sum to 1", {
  set.seed(42)
  x_list <- lapply(1:15, function(i) matrix(rnorm(6), 2, 3))
  params <- matrix_mixture_kmeanspp_init(x_list, g = 2)
  expect_equal(sum(params$pi), 1, tolerance = 1e-10)
...
test_that("kmeanspp init produces SPD covariance matrices", {
  set.seed(42)
  x_list <- lapply(1:15, function(i) matrix(rnorm(6), 2, 3))
  params <- matrix_mixture_kmeanspp_init(x_list, g = 2)
  for (j in 1:2) {
...
test_that("kmeanspp init mean matrices have correct dimensions", {
  set.seed(42)
  x_list <- lapply(1:15, function(i) matrix(rnorm(6), 2, 3))
  params <- matrix_mixture_kmeanspp_init(x_list, g = 3)
  for (j in 1:3) {
    expect_equal(dim(params$M[[j]]), c(2, 3))
    expect_equal(dim(params$U[[j]]), c(2, 2))
    expect_equal(dim(params$V[[j]]), c(3, 3))
...
test_that("kmeanspp init works via mixture_fit with init='kmeans++'", {
  set.seed(123)
  x_list <- c(
...
test_that("kmeanspp init selects spread-out centers", {
  set.seed(42)
...
## ---- matrix_mixture_random_init ----

test_that("random init returns correct structure", {
  set.seed(42)
  x_list <- lapply(1:15, function(i) matrix(rnorm(6), 2, 3))
  params <- matrix_mixture_random_init(x_list, g = 2)
...
test_that("random init assigns all observations to a cluster", {
  set.seed(42)
  x_list <- lapply(1:20, function(i) matrix(rnorm(6), 2, 3))
  params <- matrix_mixture_random_init(x_list, g = 3)
  expect_length(params$cluster, 20)
  expect_true(all(params$cluster %in% 1:3))
})
