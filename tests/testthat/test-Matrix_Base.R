...
test_that("matrix_mahalanobis is zero at the mean", {
  mean_mat <- matrix(c(1, 2, 3, 4, 5, 6), nrow = 2, ncol = 3)
  row_cov <- diag(2)
  col_cov <- diag(3)
  d <- matrix_mahalanobis(mean_mat, mean_mat, row_cov, col_cov)
  expect_equal(d, 0, tolerance = 1e-8)
})

test_that("matrix_mahalanobis is positive away from mean", {
  mean_mat <- matrix(0, 2, 3)
  x <- matrix(1, 2, 3)
  row_cov <- diag(2)
  col_cov <- diag(3)
  d <- matrix_mahalanobis(x, mean_mat, row_cov, col_cov)
  expect_gt(d, 0)
})

test_that("matrix_mahalanobis with identity cov equals Frobenius norm squared", {
  set.seed(42)
  mean_mat <- matrix(0, 2, 3)
...
test_that("matrix_mahalanobis scales with covariance", {
  set.seed(123)
  x <- matrix(rnorm(6), 2, 3)
  mean_mat <- matrix(0, 2, 3)

  d1 <- matrix_mahalanobis(x, mean_mat, diag(2), diag(3))
  d2 <- matrix_mahalanobis(x, mean_mat, 2 * diag(2), diag(3))
...
test_that("matrix_variate_log_density returns finite scalar", {
  set.seed(1)
  x <- matrix(rnorm(6), 2, 3)
  mean_mat <- matrix(0, 2, 3)
  row_cov <- diag(2)
  col_cov <- diag(3)
  ld <- matrix_variate_log_density(x, mean_mat, row_cov, col_cov)
  expect_true(is.finite(ld))
  expect_length(ld, 1)
})

test_that("matrix_variate_log_density is maximized at the mean", {
  set.seed(10)
  mean_mat <- matrix(c(1, 2, 3, 4, 5, 6), 2, 3)
  row_cov <- diag(2)
  col_cov <- diag(3)
  ld_at_mean <- matrix_variate_log_density(mean_mat, mean_mat, row_cov, col_cov)
  ld_away <- matrix_variate_log_density(mean_mat + 5, mean_mat, row_cov, col_cov)
  expect_gt(ld_at_mean, ld_away)
})

test_that("matrix_variate_log_density is always negative", {
  set.seed(5)
  x <- matrix(rnorm(6), 2, 3)
  mean_mat <- matrix(0, 2, 3)
  ld <- matrix_variate_log_density(x, mean_mat, diag(2), diag(3))
  expect_lt(ld, 0)
})

## ---- matrix_variate_mixture_fit ----

test_that("matrix_variate_mixture_fit returns correct structure", {
  set.seed(42)
...
test_that("matrix_variate_mixture_fit mixing proportions sum to 1", {
  set.seed(42)
  x_list <- c(
    lapply(1:10, function(i) matrix(rnorm(6, mean = 3), 2, 3)),
    lapply(1:10, function(i) matrix(rnorm(6, mean = -3), 2, 3))
...
test_that("matrix_variate_mixture_fit recovers well-separated clusters", {
  set.seed(123)
  ...
  fit <- matrix_variate_mixture_fit(x_list, g = 2, max_iter = 50, verbose = FALSE)
...
test_that("matrix_variate_mixture_fit log-likelihood is non-decreasing", {
  set.seed(7)
...
test_that("matrix_variate_mixture_fit responsibilities sum to 1 per row", {
  set.seed(55)
  x_list <- lapply(1:12, function(i) matrix(rnorm(6), 2, 3))
  fit <- matrix_variate_mixture_fit(x_list, g = 2, max_iter = 20, verbose = FALSE)
