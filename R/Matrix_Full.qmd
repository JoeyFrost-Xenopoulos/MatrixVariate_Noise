...
matrix_mahalanobis <- function(x, mean_matrix, row_cov, col_cov) {
	row_cov <- (row_cov + t(row_cov)) / 2
	col_cov <- (col_cov + t(col_cov)) / 2
...
matrix_mixture_kmeans_init <- function(x_list, g, nstart = 10) {
	...
	for (component in seq_len(g)) {
		...
		# Regularize
		row_covariances[[component]] <- (row_cov + t(row_cov)) / 2
		col_covariances[[component]] <- (col_cov + t(col_cov)) / 2
...
matrix_variate_mixture_fit <- function(x_list, g, max_iter = 100, tol = 1e-06,
	...
	for (iteration in seq_len(max_iter)) {
		...
		for (component in seq_len(g)) {
			...
			# Regularize
			row_cov <- (row_cov + t(row_cov)) / 2 + diag(nrow(row_cov))
			...
			# Regularize
			col_cov <- (col_cov + t(col_cov)) / 2 + diag(nrow(col_cov))
