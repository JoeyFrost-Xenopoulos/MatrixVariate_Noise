...
base_plots_dir <- file.path("loglik", "plots_base_n50")

cat("\n===== Running base Viroli across size grid with increasing contamination (n=50) =====\n")
...
for (s in seq_along(size_grid)) {
  ...
  for (noise_prop in seq(0, 30, by = 1)) {
    ...
    results[[sprintf("base_prop%d_sz%d_n50", noise_prop, s - 1)]] <- future(run_base_replicates(
      x_list = sim_viroli_prop$x_list, g = 3,
      scenario_name = sprintf("base_prop%d_sz%d_n50", noise_prop, s - 1),
...
for (s in seq_along(size_grid)) {
  ...
  sz_metrics <- all_metrics[grepl(sprintf("_sz%d_n50$", s - 1), scenario)]
